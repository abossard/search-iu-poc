...
export const graphqlMiddleware = async (req: EdgeRequest, res: Response, next: NextFunction) => {
    try {
        ...
        // Use the filtered updates from previous middleware
        const mupdates = req.updates || [];
        req.body.graphqlData = [];

        if (mupdates.length === 0) {
            console.log('No updates to process in GraphQL middleware');
            next();
            return;
        }
        ...
        if (pagesToRefresh.length === 0) {
            console.log('No layout data updates found');
            next();
            return;
        }
